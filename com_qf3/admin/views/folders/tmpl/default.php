...
$canDelete = $user->authorise( 'core.delete', 'com_qf3');

if($del=$app->input->get('del', '', 'str')){
	if(!delFiles($del, $canDelete)){
		$app->redirect('index.php?option=com_qf3&view=folders', 'QF_ERR_FILE_DEL', 'error');
	}
	$app->redirect('index.php?option=com_qf3&view=folders');
}
...
echo '<h3>'.JText::_('QF_DEL_OLD_FILES').'</h3>';
echo '/components/com_qf3/assets/attachment/<br><br>';

echo ls_recursive(JPATH_SITE.'/components/com_qf3/assets/attachment', $canDelete);

function ls_recursive($dir, $canDelete)  {
   if (is_dir($dir)) {
     $files = scandir($dir,1);
     ...
     foreach ($files as $file) {
       ...
       if (is_dir($currentfile))  {
         if ($file != '.' && $file != '..')  {
         $last_dir .= "<span>".((int)$file?"(".date("d-m-Y", $file).")":"").svgfolder()."<a href=\"javascript:void(0)\" onclick=\"tgldispl(this)\">". $file."</a> (".countFiles($currentfile).") ".delLink($currentfile, $canDelete)."<br>";
           echo $last_dir;
					 echo "<span style=\"display:none\">";
							 ls_recursive($currentfile, $canDelete);
					 echo "</span></span>";
         }
       } else {
         $last_dir .= svgfile()."<a target=\"_blank\" style=\"color:grey\" href=\"http://".$_SERVER['HTTP_HOST'].str_replace(JPATH_SITE,'',$currentfile) ."\">". $file . "</a> ".delLink($currentfile, $canDelete)."<br>";
         echo $last_dir;
       }
     }
   }
...
function delLink($currentfile, $canDelete){
	if(!$canDelete) return '';

	$file = str_replace(JPATH_SITE.'/components/com_qf3/assets/attachment/','',$currentfile);
	$dlink = 'index.php?option=com_qf3&view=folders&del=';
...
function delFiles($file, $canDelete){
	if(!$canDelete || !$file) return;

	$dir = JPATH_SITE.'/components/com_qf3/assets/attachment/'.$file;
	...
	elseif(file_exists($dir)) return unlink($dir);
}
...
<script>
function tgldispl(x){
	x=jQuery(x).parent().find('span')[0].style;
